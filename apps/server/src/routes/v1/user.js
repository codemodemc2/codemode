...
const { disableRegistration, requireAuthenticated } = require("@/helpers/auth");
const Mongoose = require("mongoose");

const checkForAlts = async (id, req) => {
  let past_accounts = req.session.history || [];

  past_accounts.push(String(id));
  past_accounts = new Set(past_accounts.map((id) => String(id)));

  req.session.history = Array.from(past_accounts);

  if (past_accounts.size <= 1) return;

  let alt_accounts = await User.find({
    _id: {
      $in: Array.from(past_accounts).map((id) => Mongoose.Types.ObjectId(id)),
    },
  });

  let all_alts = new Set();

  for (let account of alt_accounts) {
    for (let alt of account.alts) {
      all_alts.add(String(alt));
    }
    all_alts.add(String(account._id));
  }

  let temp_alts = new Set([...past_accounts, ...all_alts]);
  let additional_alts = await User.find({
    _id: {
      $in: Array.from(temp_alts).map((id) => Mongoose.Types.ObjectId(id)),
    },
  });
  alt_accounts = Array.from(additional_alts);

  for (let account of alt_accounts) {
    let account_alts = new Set(all_alts);
    account_alts.delete(String(account._id));
    account.alts = Array.from(account_alts).map((id) =>
  ...
  router.post("/login", (req, res, next) => {
    passport.authenticate("local-signin", (err, user, info) => {
      ...
      req.logIn(user, async (err) => {
        ...
        await user.save();
        await checkForAlts(user._id, req);
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      try {
        ...
        user.email = newEmail;
        user.true_email = sterilizeGmail(newEmail);
        await user.save();
