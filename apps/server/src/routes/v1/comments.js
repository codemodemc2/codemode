...
module.exports = (router) => {
	...
	router.get("/comments/:id", async (req, res, next) => {
		...
		try {
			let comments = await Comment.find({ parent: id }).populate("created_by", "username", User);
			if (!comments) return next({ status: 404, message: "Comments not found" });
			...
			for (let comment of comments) {
				...
				if (comment.children.length > 0) {
					for (let child of comment.children) {
						let childComment = await Comment.findById(child).populate("created_by", "username", User);
	...
	router.post("/like-comment", async (req, res, next) => {
		...
		if (!_id || typeof (state) == "undefined") return next({ status: 400, message: "Missing required fields" });
