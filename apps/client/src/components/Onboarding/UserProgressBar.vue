      ...
      <div v-for="link in links" :key="link.id">
        <router-link :to="link.href" class="flex flex-row gap-3">
          <div
            ...
            :class="{
              'bg-brand-primary': userOnboardingStore.steps[link.id].finished,
              'after:absolute after:border after:border-brand-primary after:-right-14 after:w-14':
                userOnboardingStore.steps[link.id].finished &&
                link.id < links.length &&
                link.id < userOnboardingStore.currentStep,
          ...
          >
            <TransitionChild
              v-if="userOnboardingStore.steps[link.id].finished"
              appear
              :show="link.id < userOnboardingStore.currentStep"
...
let links = ref([
  {
    ...
    href: "/register-invited/user",
    id: 1,
  },
  {
    name: "Password",
    description: "Secure password for your admin account",
    href: "/register-invited/password",
    id: 2,
  },
]);
</script>
