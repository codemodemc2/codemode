  ...
  <div class="flex flex-col">
    ...
    >
      ...
      <div class="flex flex-1 items-center justify-between self-center">
        <div>
          <p class="text-sm text-gray-700">Admins: {{ items }}</p>
        </div>
        <div class="flex flex-row items-center gap-5">
          <div class="flex w-full flex-row items-center gap-2">
            <p class="whitespace-nowrap">Admins per page:</p>
            <input
              v-model="perPage"
...
-->
  ...
  <AddAdminDialog ref="add_admin_dialog" />
  </div>
</template>
...
import { getAdmins } from "@/helpers/api/admin";
import AddAdminDialog from "@/components/Dialogs/Admin/Users/AddAdminDialog.vue";
...
const add_admin_dialog = ref(null);
...
let openAddAdminDialog = async () => {
	console.log(add_admin_dialog);
  add_admin_dialog.value.openModal();
};

let updateUsers = async () => {
  let data = await getAdmins(page.value, perPage.value);
  users.value = [];
